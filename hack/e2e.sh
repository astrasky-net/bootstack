...
ip link del bootstack-br0 || true
ip link add bootstack-br0 type bridge
ip link set bootstack-br0 up
ip addr add 192.168.123.1/24 dev bootstack-br0
...
# Prepare networking
for i in 1 2 3; do
  ip link del bootstack-srv$i || true
  ip tuntap add dev bootstack-srv$i mode tap
  ip link set bootstack-srv$i up
  ip link set bootstack-srv$i master bootstack-br0
done

# Start VMs
for i in 1 2 3; do
  qemu-system-x86_64 -machine type=pc,accel=kvm -cpu host -smp 4 -m 8192 \
    -device virtio-net,netdev=net0,mac=52:54:00:12:34:5$i -netdev tap,id=net0,ifname=bootstack-srv$i,script=no,downscript=no \
...
machine:
  ...
  kernel:
    ...
    - name: spl
  install:
    image: ghcr.io/astrasky-net/bootstack/talos:v1.7.6
  files:
  - content: |
      [plugins]
...
talosctl gen config --with-secrets secrets.yaml bootstack https://192.168.123.10:6443 --config-patch=@patch.yaml --config-patch-control-plane @patch-controlplane.yaml
...
kubectl create ns bootstack-system
...
timeout 60 sh -c 'until kubectl get hr -A | grep bootstack; do sleep 1; done'

sleep 5

kubectl get hr -A | awk 'NR>1 {print "kubectl wait --timeout=15m --for=condition=ready -n " $1 " hr/" $2 " &"} END{print "wait"}' | sh -x
# Wait for linstor controller
kubectl wait deploy --timeout=5m --for=condition=available -n bootstack-linstor linstor-controller

# Wait for all linstor nodes become Online
timeout 60 sh -c 'until [ $(kubectl exec -n bootstack-linstor deploy/linstor-controller -- linstor node list | grep -c Online) = 3 ]; do sleep 1; done'

kubectl exec -n bootstack-linstor deploy/linstor-controller -- linstor ps cdp zfs srv1 /dev/vdc --pool-name data --storage-pool data
kubectl exec -n bootstack-linstor deploy/linstor-controller -- linstor ps cdp zfs srv2 /dev/vdc --pool-name data --storage-pool data
kubectl exec -n bootstack-linstor deploy/linstor-controller -- linstor ps cdp zfs srv3 /dev/vdc --pool-name data --storage-pool data
